...
public class Commands {
    ...
    public static boolean executePlayerCommand(MapleClient c, String[] sub, String heading) {
        MapleCharacter chr = c.getPlayer();
        if (Objects.equals(heading, Strings.SUB_GM) && !chr.isGM()) {
            chr.showError(Strings.get(Strings.EX_INCORRECT_SUB_HEADING, heading));
            return false;
        }
        switch (sub[0]) {
            case "dispose":
            ...
            default:
                if (chr.gmLevel() == 0) {
                    chr.showError("Player Command " + heading + sub[0] + " does not exist");
                }
                return false;
        }
        return true;
    ...
    public static boolean executeGMCommand(MapleClient c, String[] sub, String heading) {
    ...
    public static void executeAdminCommand(MapleClient c, String[] sub, String heading) {
        MapleCharacter player = c.getPlayer();
        switch (sub[0]) {
            case "horntail":
            ...
            case "zakum":
                ...
                break;
            default:
                player.showError("Command " + heading + sub[0] + " does not exist.");
                break;
        }
    }
